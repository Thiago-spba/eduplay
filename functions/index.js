...
function problemaDaMissao(m, serie) {
  ...
  // Resposta certa muito mais longa que as erradas entrega o gabarito pelo tamanho
  const entregaPeloTamanho = quiz.filter(q => {
    const certa = String(q.opcoes[0]).trim().length
    const maiorErrada = Math.max(...q.opcoes.slice(1).map(o => String(o).trim().length))
    return certa >= 25 && certa > maiorErrada * 1.4
  }).length
  if (entregaPeloTamanho >= 2) return `resposta certa bem mais longa que as erradas em ${entregaPeloTamanho} perguntas`
...
- As 4 opções devem ter tamanho e estilo parecidos (diferença de até ~30% no tamanho do texto). A resposta certa NÃO pode ser a mais longa nem a mais detalhada: dê às erradas o mesmo nível de detalhe, para o aluno não acertar só pelo tamanho. Nas 3 perguntas, varie a posição da opção mais longa
